...
def make_character_index(text=text, characters=CHARACTERS):
    ...
    def is_char_in_str(char, st):
        number = re.findall(char.lower(), st.lower())
        if len(number) == 0:
            return False
        else:
            return True

    for charecter in characters:
        papers = []
        if type(charecter) != str:
            for i in charecter:
                for number, line in enumerate(text):
                    if is_char_in_str(i, line):
                        papers.append(number)
            papers = sorted(set(papers))
            ans_dict[charecter[0].lower()] = papers

        else:
            for number, line in enumerate(text):
                if is_char_in_str(charecter, line):
                    papers.append(number)
            papers = sorted(papers)
            ans_dict[charecter.lower()] = papers
