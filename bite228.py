...
def create_gravatar_url(email, size=200):
    ...
    """
    email = hashlib.md5(email.strip().lower().encode('utf-8')).hexdigest()
    return GRAVATAR_URL.format(hashed_email = email, size = size)

print(create_gravatar_url("bob@pybit.es"))