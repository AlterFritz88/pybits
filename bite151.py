...
def operas_both_at_premiere(guest, composer):
    ...
    guest_dead = _get_date(composers[guest].died)
    for opera in operas:
        opera_date = _get_date(opera.date)
        if opera.author == composer and (opera_date < guest_dead) and (opera_date > guest_birth) and (_get_date(composers[composer].died) > opera_date):
            operas_list.append(opera.play)
    return operas_list

print(operas_both_at_premiere("wagner", "verdi"))