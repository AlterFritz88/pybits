...
def years_gold_value_decreased(gold_prices: str = gold_prices) -> (int, int):
    ...
    """
    gold_prices = gold_prices.replace('\n', ' ').split()

    pairs = {'year': [], 'cost': []}
    for item in gold_prices:
        items = item.split(',')
        pairs['year'].append(int(items[0].split('-')[0]))
        pairs['cost'].append(float(items[1]))


    df = pd.DataFrame.from_dict(pairs)
    df['diff'] = df['cost'].diff()

    return (df['year'][df['diff'].idxmin()], df['year'][df['diff'].idxmax()])
print(years_gold_value_decreased(gold_prices))