...
def extract_quotes(html: str = HTML) -> dict:
    ...
    text = re.findall(r'<p>\d{1,2}.*?</p>', html)
    quotes = [re.findall(r'"\b[A-Z].*?."', x)[0].replace('"', '') for x in text]
    person = [re.findall(r'-\s\b[A-Z].*?</p>', x)[0][2:].replace('</p>', '') for x in text]
    out_dict = {}
    for x, y in zip(quotes, person):
        out_dict[y] = x
    return out_dict
