...
def get_highest_earnings(earnings_mln, n=3):
    ...
    for di in earnings_mln:
        heapq.heappush(heap, di['earnings'])
    largest = heapq.nlargest(n, heap)
    ans = []
    for la in largest:
        ans.append([x for x in earnings_mln if la in x.values()][0])
    return ans

print(get_largest_number(numbers, n=3))
print(get_latest_dates(dates))

get_highest_earnings(earnings_mln, 3)